...
sentry_sdk.init(
    dsn="https://efcbd23fedb2060efb2a5abf14b37439@o4512005451218944.ingest.de.sentry.io/4512033931001936",
...
# ============================================================
# SENTRY USAGE METRICS
# ============================================================

def track_metric(name, value=1, attributes=None):
    """
    Send a non-identifying FoodLens usage metric to Sentry.

    Analytics must never interfere with the actual FoodLens app,
    so metric failures are deliberately ignored.
    """
...
def track_duration(name, duration_ms, attributes=None):
    """
    Send a request-duration metric to Sentry.
    """
...
def get_food_data():
    ...
    # Do not store the actual barcode/query in Sentry.
    # Only record the type of request.
    query_type = "barcode" if query.isdigit() else "text_search"

    track_metric(
        "foodlens.search.requested",
        attributes={
    ...
    if query.isdigit():
        ...
        if status == 404:
            ...
            track_duration(
                "foodlens.lookup.duration",
                (time.time() - request_started) * 1000,
                attributes={"query_type": "barcode"},
            )
        ...
        response_data = build_product_response(
            product,
            barcode=barcode
        )
        ...
        track_metric(
            "foodlens.score_returned",
            attributes={
                "availability": (
                    "available"
                    if response_data["foodlens"]["score"] is not None
                    else "unavailable"
                ),
            },
        )
    ...
    if data is None:
        ...
        track_duration(
            "foodlens.lookup.duration",
            (time.time() - request_started) * 1000,
            attributes={"query_type": "text_search"},
        )
